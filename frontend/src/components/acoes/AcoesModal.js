...
const AcoesModal = ({ open, onClose, marketId }) => {
	...
	useEffect(() => {
		const fetchUser = async () => {
			if (!marketId) return;
			try {
				const { data } = await api.get(`/acoes/${marketId}`);
				setAcoe(data);
	...
	const handleSaveAcoe = async values => {
		...
		try {
			if (marketId) {
				await api.put(`/acoes/${marketId}`, acoeData);
				handleClose();
			} else {
			 await api.post("/acoes", acoeData);
