...
const CaixaModal = ({ open, onClose, marketId }) => {
	...
	useEffect(() => {
		const fetchUser = async () => {
			if (!marketId) return;
			try {
				const { data } = await api.get(`/caixa/${marketId}`);
				setAcoe(data);
	...
	const handleSaveAcoe = async values => {
		...
		try {
			if (marketId) {
				await api.put(`/caixa/${marketId}`, acoeData);
				handleClose();
			} else {
			 await api.post("/caixa", acoeData);
							...
							<DialogContent dividers>
								...
								<div>
									<Field
										...
										label={'Descrição'}
										name="description"
										placeholder="Carro Ranger Hover..."
										variant="outlined"
										margin="dense"
										fullWidth
										className={classes.textField}
