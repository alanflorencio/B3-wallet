...
const CryptoModal = ({ open, onClose, marketId }) => {
	...
	useEffect(() => {
		const fetchUser = async () => {
			if (!marketId) return;
			try {
				const { data } = await api.get(`/crypto/${marketId}`);
				setAcoe(data);
	...
	const handleSaveAcoe = async values => {
		...
		try {
			if (marketId) {
				await api.put(`/crypto/${marketId}`, acoeData);
				handleClose();
			} else {
			 await api.post("/crypto", acoeData);
